...
async def get_analytics(current_user: str = Depends(get_current_user)):
    try:
        ...
        if not user or user.get("role") != "admin":
            logger.warning(f"Unauthorized attempt by {current_user} to access analytics data.")
        ...
        finance_checks = [event for event in analytics_data if "checked financing options" in event["event"]]
        ...
        for event in logins:
            username = event["event"].split(" ")[1]  # Extract username from the event message
            analytics_summary["logins_per_user"][username] = analytics_summary["logins_per_user"].get(username, 0) + 1
        
        for event in finance_checks:
            username = event["event"].split(" ")[1]  # Extract username from the event message
            analytics_summary["financing_checks_per_user"][username] = analytics_summary["financing_checks_per_user"].get(username, 0) + 1

        logger.info(f"Analytics data fetched successfully for admin user: {current_user}")
