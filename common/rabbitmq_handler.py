...
def publish_message(queue, message, token=None):
    try:
        logger.info(
            f"Publishing message to RabbitMQ at {rabbitmq_uri} for queue: {queue}"
        )
        connection = pika.BlockingConnection(pika.URLParameters(rabbitmq_uri))
        channel = connection.channel()
        channel.queue_declare(queue=queue)
        properties = (
            pika.BasicProperties(headers={"Authorization": token}) if token else None
        )
...
def publish_analytics_event(queue, message, token=None):
    try:
        logger.info(
            f"Publishing analytics event to RabbitMQ at {rabbitmq_uri} for queue: {queue}"
        )
        connection = pika.BlockingConnection(pika.URLParameters(rabbitmq_uri))
        channel = connection.channel()
        channel.queue_declare(queue=queue)
        ...
        raise HTTPException(
            status_code=500, detail="Failed to publish analytics event to RabbitMQ"
        )
